  ...
  update_tx_db: function(coin, start, end, timeout, cb) {
    ...
    lib.syncLoop(((end - start)==0?1:(end-start)), function (loop) {
        ...
        if (blockhash) {
          lib.get_block(blockhash, function(block) {
            if (block && block.name != "RpcError") {
              ...
              console.log('block not found: %s. Error: %s', (process.env['func'] != 'check' ? start + x : JSON.parse("["+ process.env['workload']+"]")[x]), blockhash);
              loop.next();
            }
          });
        } else {
      ...
      Tx.findOne({}).sort({blockindex: 'desc'}).exec(function(err, txs){
          console.log("update_tx_db finished. Updating stats.Last to set %s", txs.blockindex);
        Stats.updateOne({coin: coin}, {
          last: txs.blockindex,
