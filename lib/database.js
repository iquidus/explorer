  ...
  update_tx_db: function(coin, start, end, timeout, cb) {
    try {
      /*setInterval(function(){
        ...
        var rss = humanFileSize(mem.rss)
        var htot = humanFileSize(mem.rss)
        var huse = humanFileSize(mem.rss)
        var ext = humanFileSize(mem.rss)
        workerLog.info("Rss: %s, heapTotal: %s, heapUsed: %s, external: %s", rss, htot, huse, ext);
      }, 5000);*/
    //var complete = false;
    //var last = start;
    lib.syncLoop(((end - start)==0?1:(end-start)), function (loop) {
      var x = loop.iteration();
      if(typeof process.env['workload'] == "string"){
        process.env['workload'] =[process.env['workload']]; 
      }
        ...
        if (blockhash) {
          lib.get_block(blockhash, function(block) {
            if (block && block.name != "RpcError") {
              ...
              workerLog.info("Block: %s has %s txes", block.height, block.tx.length);
              process.send({
                msg: 'blkupdate',
                pid: process.pid,
              ...
              lib.syncLoop(block.tx.length, function (subloop) {
                ...
                Tx.findOne({txid: block.tx[i]}, function(err, tx) {
                  if(tx) {
                    ...
                    save_tx(block.tx[i], function(err){
                      if (err) {
                        onlyConsole.log("There is an error with Block: %s, TX: %s", block.height, block.tx[i]);
                        onlyConsole.trace(err);
                      } else {
                        workerLog.info('%s: %s', block.height, block.tx[i]);
                      }
                      //last = block.height;
                      tx = null;
                      subloop.next();
                    ...
                    workerLog.info('%s doesn\'t have enough confirmations', block.height);
                    subloop.break();
                  }
                });
              }, function(){
              ...
              workerLog.error('block not found: %s. Error: %s', (process.env['func'] != 'check' ? start + x : JSON.parse("["+ process.env['workload']+"]")[x]), blockhash);
              loop.next();
            }
          });
        } else {
      ...
      Tx.findOne({}).sort({blockindex: 'desc'}).exec(function(err, txs){
        workerLog.info("update_tx_db finished. Updating stats.Last to set %s", txs.blockindex);
        Stats.updateOne({coin: coin}, {
          last: txs.blockindex,
    ...
    logger.error("There was an error, trace coming");
    logger.trace(e);
    logger.error("Trace complete. returning to cb()");
    return cb();
  }
  },
