...
function process_richlist(list, address, total, cb) {
  if (list.length > 0) {
    if (total >= list[list.length-1].total) {
      list.push({addresses: address, total: total, rank: 3});
      return cb(list);
    } else if (list.length < 100){
      list.push({addresses: address, total: total, rank: 2});
      return cb(list);
    } else {
      return cb(list);
    }
  } else {
    list.push({addresses: address, total: total, rank: 1});
    return cb(list);
  }
  /*
  if (list.length > 0) {
    if (total >= list[list.length-1].total) {
      if (total >= list[0].total) {
        list.unshift({addresses: address, total: total, rank: 1});
        return cb(list);  
      } else {
        list.push({addresses: address, total: total, rank: 1});
        return cb(list);
  ...
  /*if ( list.length < 1 ) {
    ...
    lib.syncLoop(list.length, function (loop) {
      var i = loop.iteration();
      if (parseFloat(total) <= parseFloat(list[i].total)) {
        if ( i == list.length + 1 && list.length < 100) {
          list.push({addresses: address, total: total, rank: i+1});
          loop.break(true);
          loop.next();
        } else {
          loop.next();
        ...
        if (list.length == 100) {
          list.splice(i, 0, {addresses: address, total: total, rank: i+1});
          list[100] = null;
          loop.break(true);
          loop.next();
        } else {
    ...
    if (address) {
      ...
      is_unique(tx_array, txid, function(unique, index) {
        if (unique == true) {
          ...
          if (hash != 'coinbase') {
            find_richlist(settings.coin, function(richlist){
              if (richlist) {
      ...
      if (hash != 'coinbase') {
        find_richlist(settings.coin, function(richlist){
          if (richlist) {
            //console.log(richlist);
            f_richlist(richlist.balance, hash, received, function(rl_balance){
              f_richlist(richlist.received, hash, received, function(rl_received){
                Richlist.update({coin: settings.coin}, { 
                  received : sort_richlist(rl_received),
                  balance: sort_richlist(rl_balance),
