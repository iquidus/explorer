  ...
  find_address(hash, function(address) {
    ...
    if ( hash == 'coinbase' ) {
      ...
      if (address) {
        ...
        lib.is_unique(tx_array, txid, function(unique, index) {
          lib.convert_to_satoshi(amount, function(amount_sat) {
            if (unique == true) {
              tx_array.push(txid);
            ...
            if (type == 'vin') {
              sent = sent + amount_sat;
            } else {
              received = received + amount_sat;
            }         
            Address.update({a_id:hash}, {
              txs: tx_array,
              received: received,
        ...
        if (block) {
          lib.prepare_vin(tx, function(vin) {
            lib.syncLoop(vin.length, function (loop) {
              ...
              lib.prepare_vout(tx.vout, txid, function(vout) {
...
module.exports = {
  ...
  get_distribution: function(richlist, stats, cb){
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      var i = loop.iteration();
      var count = i + 1;
      var percentage = (richlist.balance[i].balance / stats.supply) * 100;
      if (count <= 25 ) {
        distribution.t_1_25.percent = distribution.t_1_25.percent + percentage;
        distribution.t_1_25.total = distribution.t_1_25.total + richlist.balance[i].balance;
      }
      if (count <= 50 && count > 25) {
        distribution.t_26_50.percent = distribution.t_26_50.percent + percentage;
        distribution.t_26_50.total = distribution.t_26_50.total + richlist.balance[i].balance;
      }
      if (count <= 75 && count > 50) {
        distribution.t_51_75.percent = distribution.t_51_75.percent + percentage;
        distribution.t_51_75.total = distribution.t_51_75.total + richlist.balance[i].balance;
      }
      if (count <= 100 && count > 75) {
        distribution.t_76_100.percent = distribution.t_76_100.percent + percentage;
        distribution.t_76_100.total = distribution.t_76_100.total + richlist.balance[i].balance;
      }
      loop.next();
    }, function(){
                  ...
                  if(tx) {
                    ...
                    save_tx(block.tx[i], function(err){
                      if (err) {
                        ...
                        //var f_timer = new Date().getTime(); 
                        //var diff = f_timer - s_timer;
