...
module.exports = {
  ...
  update_richlist: function (list, cb) {
    if (list == 'received') {
      Address.find({}).sort({ received: -1 }).limit(100).exec(function (err, addresses) {
      ...
      Address.find({}).sort({ balance: -1 }).limit(100).exec(function (err, addresses) {
        Richlist.update({ coin: settings.coin }, {
          balance: addresses,
        }, function () {
          return cb();
  ...
  get_last_txs: function (count, min, cb) {
    Tx.find().sort({ blockindex: -1 }).limit(count).exec(function (err, txs) {
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
