...
module.exports = {
  ...
  get_rawtransaction: function(hash, cb) {
    if (settings.use_rpc) {
      rpcCommand([{method:'getrawtransaction', parameters: [hash, true]}], function(response){
        //Todo: remove console.log. Was placed here for debugging
        //console.log(`${response.height} => ${JSON.stringify(response)}`)
        return cb(response);
      });
    } else {
