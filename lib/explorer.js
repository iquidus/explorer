...
module.exports = {
      ...
      if (settings.nethash == 'netmhashps') {
        client.command([{method:'getmininginfo', parameters: []}], function(err, response){
          if(err){console.log('Error: ', err); }
            ...
            if (response[0].netmhashps) {
              if (settings.nethash_units == 'K') {
                return cb((response[0].netmhashps * 1000).toFixed(4));
              } else if (settings.nethash_units == 'G') {
                return cb((response[0].netmhashps / 1000).toFixed(4));
              } else if (settings.nethash_units == 'H') {
                return cb((response[0].netmhashps * 1000000).toFixed(4));
              } else if (settings.nethash_units == 'T') {
                return cb((response[0].netmhashps / 1000000).toFixed(4));
              } else if (settings.nethash_units == 'P') {
                return cb((response[0].netmhashps / 1000000000).toFixed(4));
              } else {
                return cb(response[0].netmhashps.toFixed(4));
              }
            } else {
              return cb('-');
            }
        ...
        client.command([{method:'getnetworkhashps', parameters: []}], function(err, response){
          if(err){console.log('Error: ', err); }
            ...
            if (response[0]) {
              if (settings.nethash_units == 'K') {
                return cb((response[0] * 1000).toFixed(4));
              } else if (settings.nethash_units == 'G') {
                return cb((response[0] / 1000).toFixed(4));
              } else if (settings.nethash_units == 'H') {
                return cb((response[0] * 1000000).toFixed(4));
              } else if (settings.nethash_units == 'T') {
                return cb((response[0] / 1000000).toFixed(4));
              } else if (settings.nethash_units == 'P') {
                return cb((response[0] / 1000000000).toFixed(4));
              } else {
                return cb(response[0].toFixed(4));
              }
            } else {
              return cb('-');
            }
  ...
  get_difficulty: function(cb) {
    if (settings.use_rpc) {
      client.command([{method:'getdifficulty', parameters: [parseInt(height)]}], function(err, response){
        if(err){console.log('Error: ', err); }
        else{
          if(response[0].name == 'RpcError'){
            return cb('There was an error. Check your console.');
  ...
  get_maxmoney: function(cb) {
    if (settings.use_rpc) {
      client.command([{method:'getmaxmoney', parameters: []}], function(err, response){
        if(err){console.log('Error: ', err); }
        else{
          if(response[0].name == 'RpcError'){
            return cb('There was an error. Check your console.');
  ...
  get_maxvote: function(cb) {
    if (settings.use_rpc) {
      client.command([{method:'getmaxvote', parameters: []}], function(err, response){
        if(err){console.log('Error: ', err); }
        else{
          if(response[0].name == 'RpcError'){
            return cb('There was an error. Check your console.');
