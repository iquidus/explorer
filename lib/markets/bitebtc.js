        ...
        if (body.success) {
            ...
            if (body['buy'].length > 0){
                for (var i = 0; i < body['buy'].length; i++) {
                    var order = {
                        amount: parseFloat(body.buy.toFixed(8),
                        price: parseFloat(body.buy.toFixed(8),
                        total: (parseFloat(body.buy.toFixed(8) * parseFloat(body.buy).toFixed(8)
                    }
                    buy.push(order);
                }
            }
            if (body['sell'].length > 0) {
                for (var x = 0; x < body['sell'].length; x++) {
                    var order = {
                        amount: parseFloat(body.sell).toFixed(8),
                        price: parseFloat(body.sell).toFixed(8),
                        total: (parseFloat(body.sell).toFixed(8) * parseFloat(body.sell).toFixed(8)
                    }
                    sell.push(order);
                }
            }
