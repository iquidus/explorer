...
function get_summary(coin, exchange, cb) {
  var req_url = base_url + 'GetTicker';
...
function get_trades(coin, exchange, cb) {
  var req_url = base_url + 'GetMarketHistory&market=' + exchange + '_' + coin;
  request({uri: req_url, json: true}, function (error, response, body) {
    if (body.error) {
...
function get_orders(coin, exchange, cb) {
  var req_url = base_url + 'GetOrderBook&market=' + exchange + '_' + coin + '&type=both&depth=50';
  request({uri: req_url, json: true}, function (error, response, body) {
    if (body.error) {
      return cb(body.error, []);
    } else {
...
function get_chartdata(coin, exchange, cb) { 
  ...
  start = end - 86400; 
  var req_url = base_url + 'GetMarketSummary&market=' + exchange + '_' + coin + '&start=' + start + '&end=' + end + '&period=1800';
