...
exports.reloadSettings = function reloadSettings() {
  ...
  for(var i in settings) {
    ...
    if (exports[i] !== undefined) {
      exports[i] = process.env[i] || settings[i] || exports[i];
    } else {
      console.warn("Unknown Setting: '" + i + "'. This setting doesn't exist or it was removed");
    }

    // sensitive information overrides
    if (process.env.MONGO_DB_URI != undefined) {
      exports.dbsettings = mongoUri.parse(process.env.MONGO_DB_URI);
    }
    if (process.env.EQUIBIT_CORE_URL != undefined) {
      const url = process.env.EQUIBIT_CORE_URL.split(':', 1);
      exports.wallet.host = url[0];
      exports.wallet.port = url[1];
    }
    if (process.env.EQUIBIT_CORE_USERNAME != undefined) {
      exports.wallet.user = process.env.EQUIBIT_CORE_USERNAME;
    }
    if (process.env.EQUIBIT_CORE_PASSWORD != undefined) {
      exports.wallet.pass = process.env.EQUIBIT_CORE_PASSWORD
    }
  }
